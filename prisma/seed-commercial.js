// One-off script — adds a full-spec commercial vehicle (Tata Ace Gold pickup)
// so the "Commercial" tab has demo data to show. Safe to run more than once
// (uses upsert everywhere) and does not touch any existing data.
//
// HOW TO RUN (Render Shell — no local setup needed):
//   1. Open your mk-crm-backend service on Render → "Shell" tab in the browser
//   2. Run:  node prisma/seed-commercial.js
//   3. Wait for "Done." — takes a few seconds
//
// This script assumes the updated schema.prisma (with Model.category) and
// a fresh `npx prisma generate` / `npx prisma db push` have already been
// deployed — see the message alongside this file for the exact order.

const { PrismaClient } = require('@prisma/client');
const prisma = new PrismaClient();

async function upsertCategory(name, displayOrder) {
  return prisma.fieldCategory.upsert({
    where: { name },
    update: {},
...
async function upsertField(categoryId, { name, key, dataType, unit, displayOrder }) {
  return prisma.fieldDefinition.upsert({
    where: { key },
    update: {},
    create: { categoryId, name, key, dataType, unit: unit || null, displayOrder, customerVisible: true },
  });
}

async function setValue(fieldId, variantId, { valueText, valueNumber, valueBoolean, applicability }) {
  return prisma.fieldValue.upsert({
    where: { fieldId_variantId: { fieldId, variantId } },
    update: { valueText, valueNumber, valueBoolean, applicability: applicability || 'STANDARD' },
    create: { fieldId, variantId, valueText, valueNumber, valueBoolean, applicability: applicability || 'STANDARD' },
  });
}
...
async function main() {
  ...
  const brand = await prisma.brand.upsert({
    ...
    update: {},
    create: { name: 'Tata Motors' },
  });

  let model = await prisma.model.findFirst({ where: { brandId: brand.id, name: 'Ace Gold' } });
  if (!model) {
    model = await prisma.model.create({
      data: { brandId: brand.id, name: 'Ace Gold', category: 'PICKUP' },
    });
  } else if (model.category !== 'PICKUP') {
    model = await prisma.model.update({ where: { id: model.id }, data: { category: 'PICKUP' } });
  }

  // ---- Variants ----
  async function upsertVariant(name, fuelType, transmission, exShowroomPrice) {
    const existing = await prisma.variant.findFirst({ where: { modelId: model.id, name } });
    if (existing) return existing;
    return prisma.variant.create({ data: { modelId: model.id, name, fuelType, transmission, exShowroomPrice } });
  }

  const std = await upsertVariant('STD', 'Diesel', 'Manual', 519000);
  const cx = await upsertVariant('CX', 'Diesel', 'Manual', 559000);

  // ---- Field categories (reuses existing category names if already present) ----
  ...
  // ---- Values — shared engine/dimension specs (same for both trims) ----
  ...
  // ---- Values that differ between STD and CX trims ----
  ...
  // ---- Colours (no photos hotlinked — add real ones later via Vehicle admin) ----
  ...
  for (const v of [std, cx]) {
    const existingVehicle = await prisma.vehicle.findFirst({ where: { variantId: v.id } });
    if (existingVehicle) {
      await prisma.vehicle.update({ where: { id: existingVehicle.id }, data: { colourOptionsJson: colours, imagesJson: '[]' } });
    } else {
      await prisma.vehicle.create({ data: { variantId: v.id, colourOptionsJson: colours, imagesJson: '[]', isActive: true } });
    }
  }

  console.log('Done. Tata Motors → Ace Gold (STD, CX) added under Commercial → PICKUP with full specs.');
}
